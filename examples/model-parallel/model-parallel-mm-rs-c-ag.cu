...
__global__ void binOpFunc0(int B, int S, int H, half * b, half * S0, half * S2, int comm_size, int rank) {
  ...
  S2[i0] = (S0[DIVUP((B*S*H), comm_size) * rank + i0] + b[i0%H]);
}
...
__global__ void binOpFunc1(int B, int S, int H, half * r, half * S0, half * S2, half * S3, int comm_size, int rank) {
  ...
  S3[i0] = ((curand_uniform(&curandState0) < 0.5 ? S2[i0] : (half) 0) + r[DIVUP((B*S*H), comm_size) * rank + i0]);
}

void model_parallel(int B, int S, int H, half* w, half* b, half* in, half* r, half* S4, half* S0, half* S2, half* S3, float& elapsedTimeAllGather, float& elapsedTimebinOpFunc1, float& elapsedTimebinOpFunc0, float& elapsedTimeReduceScatter, float& elapsedTimematMul0, ncclComm_t comm, cudaStream_t stream, int comm_size, int rank, cublasHandle_t cublasHandle){
  ...
  CUDACHECK(cudaEventRecord(startmodel_parallel, stream));
  ncclReduceScatter(S0, S0, DIVUP((B*S*H), comm_size), ncclHalf, ncclSum, comm, stream);
  CUDACHECK(cudaEventRecord(stopmodel_parallel, stream));
  CUDACHECK(cudaEventSynchronize(stopmodel_parallel));
  CUDACHECK(cudaEventElapsedTime(&elapsedTime, startmodel_parallel,stopmodel_parallel));
  ...
  CUDACHECK(cudaEventRecord(startmodel_parallel, stream));
  size_t totalThreads_1 = (size_t)DIVUP((B*S*H), comm_size);
  ...
  CUDACHECK(cudaEventRecord(startmodel_parallel, stream));
  size_t totalThreads_2 = (size_t)DIVUP((B*S*H), comm_size);
  ...
  CUDACHECK(cudaEventRecord(startmodel_parallel, stream));
  ncclAllGather(S3, S4, DIVUP((B*S*H), comm_size), ncclHalf, comm, stream);
  CUDACHECK(cudaEventRecord(stopmodel_parallel, stream));
  CUDACHECK(cudaEventSynchronize(stopmodel_parallel));
  CUDACHECK(cudaEventElapsedTime(&elapsedTime, startmodel_parallel,stopmodel_parallel));
...
int main(int argc, char** argv){
  ...
  for (int iter_B = 0; iter_B< sizeof(array_B)/sizeof(array_B[0]);iter_B++) {
    ...
    // Outputs
    half* S4;
    CUDACHECK(cudaMalloc(&S4, (B*S*H) * sizeof(half)));
    ...
    CUDACHECK(cudaMalloc(&S2, DIVUP((B*S*H), comm_size) * sizeof(half)));
    CUDACHECK(cudaMalloc(&S3, DIVUP((B*S*H), comm_size) * sizeof(half)));
    ...
    for(int iter = 0; iter < epochs; iter++) {
      model_parallel(B, S, H, w, b, in, r, S4, S0, S2, S3, elapsedTimeAllGather, elapsedTimebinOpFunc1, elapsedTimebinOpFunc0, elapsedTimeReduceScatter, elapsedTimematMul0, comm, stream, comm_size, rank, cublasHandle); 
    }
    CUDACHECK(cudaFree(w));
    CUDACHECK(cudaFree(b));
    CUDACHECK(cudaFree(in));
    CUDACHECK(cudaFree(r));
    CUDACHECK(cudaFree(S4));
    CUDACHECK(cudaFree(S0));
    CUDACHECK(cudaFree(S2));
    CUDACHECK(cudaFree(S3));
    if (rank == 0) 
      printf("{B: %ld, S: %ld, H: %ld, Epochs: %d, AllGather: %f, binOpFunc1: %f, binOpFunc0: %f, ReduceScatter: %f, matMul0: %f, Total: %f}\n", B, S, H, epochs, elapsedTimeAllGather, elapsedTimebinOpFunc1, elapsedTimebinOpFunc0, elapsedTimeReduceScatter, elapsedTimematMul0, elapsedTimeAllGather + elapsedTimebinOpFunc1 + elapsedTimebinOpFunc0 + elapsedTimeReduceScatter + elapsedTimematMul0);
  }
  MPI_Finalize();
}
