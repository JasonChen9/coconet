...
__global__ void binOpFunc0(int B, int S, int H, half * r, half * S0, half * S2, int comm_size, int rank) {
  ...
  S2[i0] = (S0[DIVUP((B*S*H), comm_size) * rank + i0] + r[DIVUP((B*S*H), comm_size) * rank + i0]);
}

void pipeline_parallel(int B, int S, int H, half* w, half* b, half* in, half* r, half* S3, half* S0, half* S2, float& elapsedTimeAllGather, float& elapsedTimebinOpFunc0, float& elapsedTimeReduceScatter, float& elapsedTimematMul0, ncclComm_t comm, cudaStream_t stream, int comm_size, int rank, cublasHandle_t cublasHandle){
  ...
  CUDACHECK(cudaEventRecord(startpipeline_parallel, stream));
  ncclAllGather(S2, S3, DIVUP((B*S*H), comm_size), ncclHalf, comm, stream);
  CUDACHECK(cudaEventRecord(stoppipeline_parallel, stream));
  CUDACHECK(cudaEventSynchronize(stoppipeline_parallel));
  CUDACHECK(cudaEventElapsedTime(&elapsedTime, startpipeline_parallel,stoppipeline_parallel));
...
int main(int argc, char** argv){
  ...
  for (int iter_B = 0; iter_B< sizeof(array_B)/sizeof(array_B[0]);iter_B++) {
    ...
    for(int iter = 0; iter < epochs; iter++) {
      pipeline_parallel(B, S, H, w, b, in, r, S3, S0, S2, elapsedTimeAllGather, elapsedTimebinOpFunc0, elapsedTimeReduceScatter, elapsedTimematMul0, comm, stream, comm_size, rank, cublasHandle); 
    }
    ...
    if (rank == 0) 
      printf("{B: %ld, S: %ld, H: %ld, Epochs: %d, AllGather: %f, binOpFunc0: %f, ReduceScatter: %f, matMul0: %f, Total: %f}\n", B, S, H, epochs, elapsedTimeAllGather, elapsedTimebinOpFunc0, elapsedTimeReduceScatter, elapsedTimematMul0, elapsedTimeAllGather + elapsedTimebinOpFunc0 + elapsedTimeReduceScatter + elapsedTimematMul0);
  }
  MPI_Finalize();
}
