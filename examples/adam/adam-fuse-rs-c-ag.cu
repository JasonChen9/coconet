#include "header.h"
void adam(int N, float lr, float beta1, float beta2, float* g, float* w, float* m, float* v, float& elapsedTimeFusedAllReduce, ncclComm_t comm, cudaStream_t stream, int comm_size, int rank){
...
int main(int argc, char** argv){
  ...
  for (int __i = 10; __i < 30; __i++) {
    ...
    for(int iter = 0; iter < epochs; iter++) {
      adam(N, lr, beta1, beta2, g, w, m, v, elapsedTimeFusedAllReduce, comm, stream, comm_size, rank); 
    }
