...
try:
    currDir = os.getcwd()
    print("Running Pipeline Parallel Experiments")

    # compile_nccl(nccl_path)
    os.chdir("../examples/pipeline-parallel")
    make_clean()
    eval_binary("pipeline-parallel-ar-p2p-c")
    eval_binary("pipeline-parallel-rs-p2p-c-ag")
    make_binary("pipeline-parallel-ol-rs-fuse-p2p-c-ag.cu")
    compile_nccl(nccl_path)
    os.chdir("../examples/pipeline-parallel")
    eval_binary("pipeline-parallel-ol-rs-fuse-p2p-c-ag")
    os.chdir(currDir)
